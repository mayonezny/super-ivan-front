...
const addPostSchema = yup.object({
  ...
  content: yup.string().min(20, 'Содержимое поста должно быть не менее 20 символов!').max(1000, 'Обалдеть какое здоровое содержание!').required('Пожалуйста, укажите содержание поста'),
...
const AddPostModal = ({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) => {
  ...
  const handlePostUpload = async (data: postUploadInterface) => {
